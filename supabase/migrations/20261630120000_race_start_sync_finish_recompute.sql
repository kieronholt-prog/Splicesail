-- Align race start updates (admin schedule + RO start signals) and recompute finish timing once per change.
...
as $$
begin
  if coalesce(current_setting('splice.skip_finish_recompute', true), '') = 'on' then
    return new;
  end if;
...
-- Single entry point for RO Apply and admin race start edits (keeps races.scheduled_at + fleet signals aligned).
...
begin
  ...
  if p_fleet_id = v_primary_id then
    ...
    loop
      if v_fleet.start_signal_at is not null then
        v_f_ms := extract(epoch from v_fleet.start_signal_at) * 1000.0;
      elsif v_old_scheduled is not null then
        v_f_ms :=
...
comment on function public.apply_race_fleet_start_signal(uuid, uuid, timestamptz) is
  'RO/admin: persist fleet start signal, sync primary fleet to races.scheduled_at, refresh stored finish timing once.';

revoke all on function public.apply_race_fleet_start_signal(uuid, uuid, timestamptz) from public;
grant execute on function public.apply_race_fleet_start_signal(uuid, uuid, timestamptz) to authenticated;
