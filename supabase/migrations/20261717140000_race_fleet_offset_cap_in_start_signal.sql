-- Ensure constraint allows pursuit / RO-recalculated offsets (idempotent if 20261716120000 already ran).
-- Cap apply_race_fleet_start_signal so recalculated offsets never exceed the check.
...
begin
  ...
  if p_fleet_id = v_primary_id then
    ...
    loop
      ...
      v_offset := least(
        v_max_offset,
        greatest(0, round((v_f_ms - v_start_ms) / 60000.0))
      );
    ...
    if v_offset is not null then
      v_offset := least(v_max_offset, v_offset);
