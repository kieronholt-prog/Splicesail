...
export function StravaActivityPicker({
  ...
  if (loading) return <p className="mt-3 text-sm text-splice-ocean">Loading Strava activities…</p>;
  if (error) return <p className="mt-3 text-sm text-red-700">{error}</p>;

  return (
    <div className="mt-3">
      <p className="text-sm text-splice-ocean dark:text-splice-water">Linked as {linkedName}</p>
        ...
        {activities.map((a) => (
          <li key={a.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
            <div>
              ...
              <p className="text-xs text-splice-blue">{new Date(a.start_date).toLocaleString()}</p>
            </div>
            <form action={createAction}>
              ...
              <button
                type="submit"
                className="rounded-lg border border-splice-navy px-3 py-1.5 text-sm dark:border-splice-foam"
              >
                Use track
              </button>
            </form>
      ...
      {activities.length === 0 ? (
        <p className="text-sm text-splice-ocean">No recent Sail or Windsurf activities on Strava.</p>
      ) : null}
    </div>
  );
}
